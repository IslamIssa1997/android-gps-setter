    ...
    private fun hookProviderDispatch(classLoader: ClassLoader) {
        ...
        for (name in classNames) {
            ...
            protectedHook(method) { chain ->
                ...
                // Resolve the app this registration is really serving -- including the app behind a
                // gms/fused or system ("android") proxy, via its WorkSource/attribution -- so we only
                // fake genuine targets. Without this we would fake gms/"android" wholesale just for
                // holding the registration, spilling the spoof onto every fused reader (and our own
                // blue dot). Fall back to the direct holder if nothing deeper resolves.
                val packages = CallerIdentity.packagesFrom(listOf(registration))
                    .ifEmpty { listOfNotNull(registrationPackage(registration)).toSet() }
                val target = packages.firstOrNull { settings.shouldSpoof(it) }
                val doSpoof = settings.spoofAllScoped || target != null
                ...
                if (faked != null) {
                    ...
                    args[0] = faked
                    if (settings.isVerboseLogging) {
                        module.log(Log.DEBUG, TAG, "system: delivered fake to ${target ?: if (settings.spoofAllScoped) "all" else "?"}")
                    }
                }
                chain.proceed(args)
            }
