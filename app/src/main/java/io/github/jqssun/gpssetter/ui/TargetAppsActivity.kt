...
class TargetAppsActivity : AppCompatActivity() {
    ...
    // Restore is attempted at most once per screen visit, so re-reads never re-trigger the popup.
    ...
    private fun refreshScope() {
        ...
        lifecycleScope.launch {
            ...
            if (current != null) {
                ...
                applyFilter()
                // Apps the user hooked but that are no longer in the live scope -- LSPosed can reset
                // a module's scope to its shipped default on an update. Offer to restore them.
                val missing = PrefManager.hookedApps - current
                if (missing.isNotEmpty() && !restoreAttempted) {
                    restoreAttempted = true
                    restoreMissingHooks(service, missing)
                }
            }
        }
    }
    ...
    /**
     * Re-requests scope, in one approval, for apps the user previously hooked that have dropped out
     * of the live scope. Only ever adds back what the user explicitly chose before.
     */
    private fun restoreMissingHooks(service: XposedService, missing: Set<String>) {
        val callback = object : XposedService.OnScopeEventListener {
            override fun onScopeRequestApproved(approved: MutableList<String>) {
                scoped.addAll(approved)
                approved.forEach {
                    runCatching { Runtime.getRuntime().exec(arrayOf("su", "-c", "am force-stop $it")) }
                }
                runOnUiThread {
                    applyFilter()
                    showToast(getString(R.string.scope_restored, approved.size))
                }
            }
            override fun onScopeRequestFailed(message: String) {
                Timber.w("scope restore failed: %s", message)
            }
        }
        lifecycleScope.launch(Dispatchers.IO) {
            runCatching { service.requestScope(missing.toList(), callback) }
                .onFailure { Timber.w(it, "scope restore threw") }
        }
    }
    ...
    private fun removeScope(pkg: String) {
        ...
        lifecycleScope.launch(Dispatchers.IO) {
            runCatching { service.removeScope(listOf(pkg)) }
                .onSuccess {
                    scoped.remove(pkg)
                    PrefManager.hookedApps = PrefManager.hookedApps - pkg
