...
object XposedPrefs {
    ...
    /**
     * Comma-separated packages the user has upgraded to the hook method ("Add to scope"). Our own
     * record of them, so if LSPosed resets the module's scope to its shipped default on an update,
     * the app can offer to restore them. Distinct from the live LSPosed scope, which is authoritative.
     */
    const val HOOKED_APPS = "hooked_apps"
    ...
    object Defaults {
        ...
        // Default OFF: spoof only apps the user explicitly targets. When this was ON, the
        // system-side hook faked location for EVERY app -- gms, "android", and GPS Setter's own
        // blue dot -- and it short-circuited the per-target check entirely. Per-app is the sane,
        // expected default; "spoof everything" should be an explicit opt-in.
        const val SPOOF_ALL_SCOPED = false
