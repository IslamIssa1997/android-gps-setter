...
class MainViewModel @Inject constructor(
    ...
    // Live getters, not cached vals -- otherwise the map reads a value frozen at construction and
    // a later change in settings (map type, position, play state) never reaches it.
    val getLat get() = prefManger.getLat
    val getLng get() = prefManger.getLng
    val isStarted get() = prefManger.isStarted
    val mapType get() = prefManger.mapType
    // A pin exists (placed at least once) -- show the red marker on reopen even when not playing.
    val hasPin get() = prefManger.hasPin
    ...
    /**
     * Whether the module is actually loaded by an Xposed framework. Derived from the libxposed
     * service binding, which only succeeds for modules the framework has enabled -- so it no longer
     * needs the self-hook on [updateXposedState] that the legacy module used for detection.
     */
    val isXposed: LiveData<Boolean> = App.xposedService.map { it != null }
    ...
    fun checkNow() {
        viewModelScope.launch {
            checkUpdates.getLatestRelease(force = true).collect { _manualResult.emit(it) }
        }
    }
    ...
    private fun downloadUpdate(context: Context, url: String, fileName: String) = viewModelScope.launch {
        ...
        // DownloadManager's ACTION_DOWNLOAD_COMPLETE is sent by the download provider, not the
        // system server, so on Android 14 (targetSdk 34) a NOT_EXPORTED receiver never gets it and
        // the download appears to hang forever. It must be registered EXPORTED. The receiver still
        // filters by our own requestId, so a spoofed broadcast just triggers a harmless re-query.
